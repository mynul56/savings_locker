...
class SavingsBloc extends Bloc<SavingsEvent, SavingsState> {
  ...
  Future<void> _onLoadDeposits(
    ...
    emit(SavingsLoading());
    final result = await loadDepositsUseCase(
      LoadDepositsParams(uid: event.uid),
    );
    result.fold(
  ...
  Future<void> _onCreateDeposit(
    ...
    result.fold((failure) => emit(SavingsFailure(failure.message)), (deposit) {
      // If we were loaded before, append the new deposit and emit updated list
      if (currentState is SavingsLoaded) {
        final updatedDeposits = List<DepositEntity>.from(currentState.deposits)
          ..insert(0, deposit);
  ...
  Future<void> _onWithdrawDeposit(
    WithdrawDeposit event,
    Emitter<SavingsState> emit,
  ) async {
    final result = await withdrawDepositUseCase(
      WithdrawDepositParams(uid: event.uid, depositId: event.depositId),
    );

    result.fold((failure) => emit(SavingsFailure(failure.message)), (_) {

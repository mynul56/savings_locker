...
class GoalsBloc extends Bloc<GoalsEvent, GoalsState> {
  ...
  Future<void> _onCreateGoal(CreateGoal event, Emitter<GoalsState> emit) async {
    ...
    result.fold(
      ...
      (goal) {
        if (currentState is GoalsLoaded) {
          final updatedGoals = List<GoalEntity>.from(currentState.goals)..insert(0, goal);
  ...
  Future<void> _onContributeGoal(ContributeGoal event, Emitter<GoalsState> emit) async {
    final result = await contributeGoalUseCase(ContributeGoalParams(
      uid: event.uid,
      goalId: event.goalId,
      amount: event.amount,
    ));
